...
class FlightDataProcessor:
    ...
    def process_layover_data(self):
        ...
        # Add Region column
        self._layover_df['Layover_Region'] = self._layover_df["city"].apply(lambda x: "North" if x in set(self.north) else "South" if x in set(self.south) else "West" if x in set(self.west) else "East")

        # Parse duration components
        self._layover_df['Layover_Hour'] = self._layover_df['layover_duration'].str.extract(r'(\d+) hr').fillna(0)
        self._layover_df['Layover_Hour'] = self._layover_df['Layover_Hour'].astype(int)
        self._layover_df['Layover_Min'] = self._layover_df['layover_duration'].str.extract(r'(\d+) min').fillna(0)
        self._layover_df['Layover_Min'] = self._layover_df['mLayover_Minin'].astype(int)

        # Create duration labels
        self._layover_df["Layover_Duration_Label"] = np.where((self._layover_df['Layover_Hour'] >= 3) & (self._layover_df['hLayover_Hourr'] < 6), "Medium",
            np.where((self._layover_df['Layover_Hour'] >= 6) & (self._layover_df['Layover_Hour'] < 12), "Long", "Ultra-Long")
        )
    ...
    def process_main_data(self):
        ...
        self._main_df['Duration_Hour'] = self._main_df['travel_duration'].str.extract(r'(\d+) hr').fillna(0).astype(int)
        self._main_df['Duration_Min'] = self._main_df['travel_duration'].str.extract(r'(\d+) min').fillna(0).astype(int)
        # Extract Day and Month from dates
        self._main_df['search_date'] = pd.to_datetime(self._main_df['search_date'])
        departure_dates = self._main_df['departure_date'].str.extract(r', (?P<month>\w+) (?P<day>\d+)')
        departure_dates['month'] = departure_dates['month'].map(lambda x: self.months.index(x) + 1)
        departure_dates['year'] = 2025
        self._main_df['departure_date'] = pd.to_datetime(departure_dates[['year', 'month', 'day']])
        # Days between from Search Date and Departure Date
        self._main_df['Days_Between'] = (self._main_df['departure_date'] - self._main_df['search_date']).dt.days

        # Add Travel Duration # Add Travel Duration Label
        self._main_df['Duration_Label'] = np.where((self._main_df['Duration_Hour'] >= 3) & (self._main_df['Duration_Hour'] < 6), "Medium",
            np.where((self._main_df['Duration_Hour'] >= 6) & (self._main_df['Duration_Hour'] < 12), "Long", "Ultra-Long"))# Add Travel Duration Label
        self._main_df['Duration_Label'] = np.where((self._main_df['Duration_Hour'] >= 3) & (self._main_df['Duration_Hour'] < 6), "Medium",
            np.where((self._main_df['Duration_Hour'] >= 6) & (self._main_df['Duration_Hour'] < 12), "Long", "Ultra-Long"))
        self._main_df['Duration_Label'] = np.where((self._main_df['Duration_Hour'] >= 3) & (self._main_df['Duration_Hour'] < 6), "Medium",
            np.where((self._main_df['Duration_Hour'] >= 6) & (self._main_df['Duration_Hour'] < 12), "Long", "Ultra-Long"))
        
        # Add Layover Label
        self._main_df['Layover_Label'] = np.where(self._main_df['num_layovers'] == 0, "Direct", "Layover")

        # Add Departure Morning/Afternoon/Evening/Night Flight
        self._main_df['Departure_Time'] = pd.to_datetime(self._main_df['departure_time'], format="%H:%M:%S")
        self._main_df['Departure_Hour'] = self._main_df['Departure_Time'].dt.hour
        self._main_df['Departure_AMPM'] = np.where(self._main_df['Departure_Hour'] < 12, "AM", "PM")
        self._main_df['Departure_Time_Label'] = np.where((self._main_df['Departure_Hour'] >= 6) & (self._main_df['Departure_Hour'] < 12), "Morning",
            np.where((self._main_df['Departure_Hour'] >= 12) & (self._main_df['Departure_Hour'] < 18), "Afternoon",
            np.where((self._main_df['Departure_Hour'] >= 18) & (self._main_df['Departure_Hour'] < 24), "Evening", "Night")))
        
        # Add Arrival Morning/Afternoon/Evening/Night Flight
        self._main_df['Arrival_Time'] = pd.to_datetime(self._main_df['arrival_time'], format="%H:%M:%S")
        self._main_df['Arrival_Hour'] = self._main_df['Arrival_Time'].dt.hour
        self._main_df['Arrival_AMPM'] = np.where(self._main_df['Arrival_Hour'] < 12, "AM", "PM")
        self._main_df['Arrival_Time_Label'] = np.where((self._main_df['Arrival_Hour'] >= 6) & (self._main_df['Arrival_Hour'] < 12), "Morning",
            np.where((self._main_df['Arrival_Hour'] >= 12) & (self._main_df['Arrival_Hour'] < 17), "Afternoon",
            np.where((self._main_df['Arrival_Hour'] >= 17) & (self._main_df['Arrival_Hour'] < 21), "Evening", "Night")))
        ...
        self._main_df['Arrival_Day'] = self._main_df['arrival_date'].str.extract(r'\w+ (\d+)')
        self._main_df['Arrival_Month'] = self._main_df['arrival_date'].str.extract(r', (\w+)').map(lambda x: self.months.index(x) + 1)

        # Add Region column
        self._main_df['Departure_Region'] = self._main_df["departure_city"].apply(lambda x: "North" if x in set(self.north) else "South" if x in set(self.south) else "West" if x in set(self.west) else "East")
        self._main_df['Arrival_Region'] = self._main_df["arrival_city"].apply(lambda x: "North" if x in set(self.north) else "South" if x in set(self.south) else "West" if x in set(self.west) else "East")

        # Add Season column
        self._main_df['Departure_Season'] = self._main_df['Departure_Month'].apply(lambda x: "Winter" if x in [12, 1, 2] else "Spring" if x in [3, 4, 5] else "Summer" if x in [6, 7, 8] else "Fall")
        self._main_df['Arrival_Season'] = self._main_df['Arrival_Month'].apply(lambda x: "Winter" if x in [12, 1, 2] else "Spring" if x in [3, 4, 5] else "Summer" if x in [6, 7, 8] else "Fall")
        
        self._main_df['Seating_Class'] = self._main_df['seating_class']
        vals = ['1', '0', 'Unknown']
        self._main_df['Airline_Name'] = self._main_df['airline_name'][self._main_df.airline_name.isin(vals)==False]
