...
def accessSpecificDates(wait, driver, departure_date, return_date):
    ...
    specific_dates_button = driver.find_element(
        By.CSS_SELECTOR, "div[jsname='mAKh3e']"
    ).find_element(By.CSS_SELECTOR, "button[id='sNlbpb']")
    ...
    departure_date_input = driver.find_element(
        By.CSS_SELECTOR, "div[jsname='O3Z1Eb']"
    ).find_element(By.CSS_SELECTOR, "input[aria-label='Departure']")
    departure_date_input.click()
    time.sleep(1)

    # Enter the Departure Date
    departure_date_input.send_keys(
        departure_date
    )  # 1/5/2025 is just a placeholder date
    time.sleep(2)

    # Find the Return Date input field
    departure_date_input = driver.find_element(
        By.CSS_SELECTOR, "div[jsname='O3Z1Eb']"
    ).find_element(By.CSS_SELECTOR, "input[aria-label='Return']")
    departure_date_input.click()
    ...
    div = wait.until(
        EC.visibility_of_element_located((By.CSS_SELECTOR, "div[class='ohKsQc']"))
    )
...
def accessFlexibleDates(wait, driver, month, week_type):
    ...
    try:
        if month == "all":
            all_months_button = driver.find_element(
                By.CSS_SELECTOR, "span[data-value='0']"
            ).find_element(By.CSS_SELECTOR, "button[jsname='LgbsSe']")
            all_months_button.click()
            time.sleep(1)
    except:
        month_button = driver.find_element(
            By.CSS_SELECTOR, f"span[data-value='{button_number+1}']"
        ).find_element(By.CSS_SELECTOR, "button[jsname='LgbsSe']")
        month_button.click
    ...
    trip_duration_div = driver.find_element(
        By.CSS_SELECTOR, "div[aria-label='Trip duration']"
    )
    span = trip_duration_div.find_element(
        By.XPATH, f".//span[normalize-space(text())='{week_type}']"
    )  # span_text
...
def accessNumOfPassengers(
    wait, driver, num_adults, num_children, num_infants_in_seat, num_infants_on_lap
):
    ...
    num_of_passengers_button = driver.find_element(
        By.CSS_SELECTOR, "div[jsname='QqIbod']"
    )
    ...
    if num_adults > 1:
        add_adult_button = driver.find_element(
            By.CSS_SELECTOR, "ul[jsname='nK2kYb']"
        ).find_element(By.CSS_SELECTOR, "button[aria-label='Add adult']")
    ...
    if num_children > 0:
        add_children_button = driver.find_element(
            By.CSS_SELECTOR, "ul[jsname='nK2kYb']"
        ).find_element(By.CSS_SELECTOR, "button[aria-label='Add child']")
    ...
    if num_infants_in_seat > 0:
        add_infants_in_seat_button = driver.find_element(
            By.CSS_SELECTOR, "ul[jsname='nK2kYb']"
        ).find_element(By.CSS_SELECTOR, "button[aria-label='Add infant in seat'")
    ...
    if num_infants_on_lap > 0:
        add_infants_on_lap_button = driver.find_element(
            By.CSS_SELECTOR, "ul[jsname='nK2kYb']"
        ).find_element(
            By.CSS_SELECTOR, "button[aria-label=aria-label='Add infant on lap']"
        )
...
def accessSeatingClass(wait, driver, class_type):
    ...
    if class_type.lower() == "Economy".lower():
        class_type_button = driver.find_element(
            By.CSS_SELECTOR, "ul[aria-label='Select your preferred seating class.']"
        ).find_element(
            By.CSS_SELECTOR, "li[data-value='1']"
        )  # 1 = Economy, 2 = Prem Econ, 3 = Business, 4 = First
        class_type_button.click()
    if class_type.lower() == "Prem Econ".lower():
        class_type_button = driver.find_element(
            By.CSS_SELECTOR, "ul[aria-label='Select your preferred seating class.']"
        ).find_element(
            By.CSS_SELECTOR, "li[data-value='2']"
        )  # 1 = Economy, 2 = Prem Econ, 3 = Business, 4 = First
        class_type_button.click()
    if class_type.lower() == "Business".lower():
        class_type_button = driver.find_element(
            ...
            By.CSS_SELECTOR, "li[data-value='3']"
        )  # 1 = Economy, 2 = Prem Econ, 3 = Business, 4 = First
        class_type_button.click()
    if class_type.lower() == "First Class".lower():
        class_type_button = driver.find_element(
            ...
            By.CSS_SELECTOR, "li[data-value='4']"
        )  # 1 = Economy, 2 = Prem Econ, 3 = Business, 4 = First
        class_type_button.click()
    time.sleep(2)
...
def changeRoundTrip(wait, driver, roundtrip):
    ...
    if not roundtrip:
        ...
        # Change to One-Way
        ticket_type = driver.find_element(
            By.CSS_SELECTOR, "ul[aria-label='Select your ticket type.']"
        ).find_element(
            By.CSS_SELECTOR, "li[data-value='2']"
        )  # data-value = '1' gives round trip else one-way
        ticket_type.click()
        time.sleep(2)
...
def accessOriginDestination(wait, driver, origin, destination):
    ...
    dropdown = wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, "ul[role='listbox'].DFGgtd"))
    )
...
def access_Flights(driver):
    ...
    view_flights_link = driver.find_element(
        By.CSS_SELECTOR, "a[aria-label='View flights']"
    )
    ...
    """View More Flights"""
    view_more_flights_button = driver.find_element(
        By.XPATH, "//button[@aria-label='View more flights']"
    )
...
def retrieveFlightDetails(driver, wait, round_trip):
    ...
    for result in all_results[:25]:
        ...
        tuple_dict = {}
        wait.until(
            EC.visibility_of_element_located((By.XPATH, ".//div[@class='JMc5Xc']"))
        )
        try:
            aria_label = result.find_element(
                By.XPATH, ".//div[@class='JMc5Xc']"
            ).get_attribute(
                "aria-label"
            )  # .find_element(By.CSS_SELECTOR, "div[@class='JMc5Xc']").get_attribute("aria-label") #result.find_element(By.CSS_SELECTOR, ".//div[@class='JMc5Xc']").get_attribute("aria-label")
        except:
            break
        if aria_label:
            cnt += 1
            ...
            dropdown_button = result.find_element(
                By.CSS_SELECTOR, "div[jsname='UsVyAb']"
            ).find_element(
                By.CSS_SELECTOR, "button[jsname='LgbsSe']"
            )  # .find_element(By.XPATH, "//Button[jsname='LgbsSe']")
            ...
            )  # VfPpkd-LgbsSe VfPpkd-LgbsSe-OWXEXe-k8QpJ VfPpkd-LgbsSe-OWXEXe-Bz112c-M1Soyc VfPpkd-LgbsSe-OWXEXe-dgl2Hf nCP5yc AjY5Oe LQeN7 nJawce OTelKf XPGpHc mAozAc   # Check if the element is enabled
            ...
            try:
                super_div_element = div_element = result.find_element(
                    By.CSS_SELECTOR, "div.m9ravf"
                ).find_element(By.CSS_SELECTOR, "div.xOMPfb.MNvMJb")

                div_element = super_div_element.find_element(
                    By.CSS_SELECTOR, "div.c257Jb.QwxBBf.eWArhb"
                )
                span_elements = div_element.find_elements(
                    By.CSS_SELECTOR, "span.Xsgmwe"
                )
                departure_airplane_type = span_elements[3].text

                div_element = super_div_element.find_element(
                    By.CSS_SELECTOR, "div.MX5RWe.sSHqwe.y52p7d"
                )
                span_elements = div_element.find_elements(
                    By.CSS_SELECTOR, "span.Xsgmwe"
                )
            ...
            for token in doc:
                print(
                    f"Token: {token.text}, Lemma: {token.lemma_}, POS: {token.pos_}, Tag: {token.tag_}, Dep: {token.dep_}, Shape: {token.shape_}, is_alpha: {token.is_alpha}, is_stop: {token.is_stop}"
                )
                ...
                while True:
                    ...
                    try:
                        all_results = driver.find_elements(
                            By.XPATH, "//ul[@class='Rk10dc']/li"
                        )
                        select_flight = all_results[cnt].find_element(
                            By.XPATH, ".//div[@class='gQ6yfe m7VU8c']"
                        )
                        select_flight.click()
                    except:
                        break
                    try:
                        view_more_flights = driver.find_element(
                            By.XPATH, ".//div[@jsname='YdtKid']"
                        ).find_element(By.XPATH, ".//button[@jsname='ornU0b']")
                        view_more_flights.click()
                    except:
                        print("")

                    time.sleep(2)
                    more_results = driver.find_elements(
                        By.XPATH, "//ul[@class='Rk10dc']/li"
                    )
                    randomHash = {}
                    for more_result in more_results:
                        try:
                            aria_label = more_result.find_element(
                                By.XPATH, ".//div[@class='JMc5Xc']"
...
def retrieveAirplaneType(driver):
    ...
    for result in all_results:
        ...
        dropdown_button.click()
        time.sleep(2)
        div_element = result.find_element(
            By.CSS_SELECTOR, "div[jsname='XxAJue']"
        ).find_element(By.CSS_SELECTOR, "div[jsname='lVbzR']")
        div_element = div_element.find_element(
            By.CSS_SELECTOR, "div.MX5RWe.sSHqwe.y52p7d"
        )
...
def extractFlightInfo(nlp_doc):
    ...
    extracted = []
    price_pattern = [[{"POS": "NUM"}, {"ORTH": "US"}, {"ORTH": "dollars"}]]
    num_stops_pattern = [[{"POS": "NUM"}, {"LEMMA": "stop"}, {"LOWER": "flight"}]]

    no_stops_pattern = [[{"LOWER": "nonstop"}]]
    ...
    num_layover_pattern = [[{"TEXT": "of"}, {"POS": "NUM"}, {"TEXT": ")"}]]
    ...
    num_of_checked_pattern = [[{"POS": "NUM"}, {"LOWER": "checked"}, {"LOWER": "bags"}]]
...
def getUserDate():
    ...
    while True:
        try:
            date_type = input(
                "Would you want a flexible or specific way to enter your departure and arrival dates? "
            )
    ...
    if date_type.lower() == "flexible":
        ...
        while True:
            try:
                user_month = input(
                    f"Which month? All {' '.join([month for month in flex_months])} "
                )
    ...
    while True:
        try:
            ...
            if num_adults > passangers:
                ...
                passangers = (
                    passangers - num_adults if passangers - num_adults >= 0 else 0
                )
                try:
                    num_child = int(
                        input(f"Enter the number of Children (Aged 2-11): ")
                    )
                    if num_child > passangers:
                        print("The maximum number of passengers is 9")
                    else:
                        passangers = (
                            passangers - num_child if passangers - num_child >= 0 else 0
                        )
                        try:
                            num_infants_in_seat = int(
                                input(f"Enter the number of Infants (In seat): ")
                            )
                            if num_infants_in_seat > passangers:
                                print("The maximum number of passengers is 9")
                            else:
                                passangers = (
                                    passangers - num_infants_in_seat
                                    if passangers - num_infants_in_seat >= 0
                                    else 0
                                )
                                try:
                                    num_infants_on_lap = int(
                                        input(f"Enter the number of Adults: ")
                                    )
                                    if num_infants_on_lap > passangers:
                                        print("The maximum number of passengers is 9")
                                    if num_infants_on_lap > num_adults:
                                        print(
                                            "The number of infants on lap cannot exceed the number of adults"
                                        )
                                    else:
                                        passangers = (
                                            passangers - num_infants_on_lap
                                            if passangers - num_infants_on_lap >= 0
                                            else 0
                                        )
                                        break
                                except:
...
def getUserSeatingClass():
    ...
    while True:
        try:
            seating_class = input(
                "Enter the Seating Class (Economy, Prem Econ, Business, First Class): "
            )
            if seating_class.lower() in [
                "economy",
                "prem econ",
                "business",
                "first class",
...
def clean_data(data):
    ...
    if data[2] == "Nonstop":
        ...
        num_stops = int(
            num_stops.group()
        )  # Use this to split the if/else logic regarding the number of layovers

        # Extract the airline
        try:
            airline = re.search(
                r"\w+\Z", data[5]
            )  # Has trouble with "American and Hawaiin" Format
            cleaned_data["Airline"] = airline.group()
            print(airline.group())
        except:
            cleaned_data["Airline"] = "Unknown"
...
def insert_flight_features(
    ...
    if int(one_way_info["Number of Stops"]) != 0:
        values = (
            one_way_info["Price"],
            flight_direction,
            one_way_info["Number of Stops"],
            airline_id,
            user_data[7],
            departure_airport_id,
            one_way_info["Departure Time"],
            one_way_info["Departure Date"],
            arrival_airport_id,
            one_way_info["Arrival Time"],
            one_way_info["Arrival Date"],
            one_way_info["Total Duration"],
            one_way_info["Number of Stops"],
            one_way_info["Number of Carry-On Bags"],
            one_way_info["Number of Checked Bags"],
            user_data[2],
            user_data[3],
            user_data[4],
            user_data[5],
...
def get_or_insert_id(
    cursor, table, column, value, return_column, additional_columns=None
):
    ...
    if result:
        ...
        if additional_columns:
            cursor.execute(
                f"INSERT INTO {table} ({column}, {', '.join(additional_columns.keys())}) VALUES (%s, {', '.join(['%s' for _ in additional_columns.keys()])}) RETURNING {return_column}",
                (value, *additional_columns.values()),
            )
        else:
            cursor.execute(
...
def pipeline(cleaned_data, user_data):
    ...
    for scraped_info in cleaned_data:
        ...
        airline_id = get_or_insert_id(
            cursor, "airlines", "airline_name", one_way_info["Airline"], "airline_id"
        )
        try:
            if scraped_info["Departure Airplane Type"]:
                departure_airplane_id = get_or_insert_id(
                    cursor,
                    "airplanes",
                    "airplane_type",
                    scraped_info["Departure Airplane Type"],
                    "airplane_id",
                )
                arrival_airplane_id = get_or_insert_id(
                    cursor,
                    "airplanes",
                    "airplane_type",
                    scraped_info["Arrival Airplane Type"],
                    "airplane_id",
                )
        except KeyError:
            continue
        departure_airport_id = get_or_insert_id(
            cursor,
            "airports",
            "airport_name",
            one_way_info["Departure Airport"],
            "airport_id",
            {"city": user_data[0]},
        )
        arrival_airport_id = get_or_insert_id(
            cursor,
            "airports",
            "airport_name",
            one_way_info["Arrival Airport"],
            "airport_id",
            {"city": user_data[1]},
        )
        ...
        if user_data[-1]:
            ...
            roundtrip_id = get_or_insert_id(
                cursor,
                "roundtrips",
                "total_price",
                cleaned_data[-1],
                "roundtrip_id",
                {"num_passengers": ttl_passengers},
            )
            connection.commit()

        flight_id = insert_flight_features(
            cursor,
            one_way_info,
            flight_direction,
            airline_id,
        ...
        connection.commit()
        if (
            len(one_way_info) > 12 and int(one_way_info["Number of Stops"]) != 0
        ):  # If there are layovers
            if int(one_way_info["Number of Stops"]) == 1:
                layover_airport_id = get_or_insert_id(
                    cursor,
                    "airports",
                    "airport_name",
                    one_way_info["Layover Airport"],
                    "airport_id",
                    {"city": one_way_info["Layover City"]},
                )
                insert_layover(
                    cursor,
                    flight_id,
                    layover_airport_id,
                    one_way_info["Layover Duration"],
                )
                connection.commit()
            else:
                for _, layover_info in one_way_info["Layovers"].items():
                    layover_airport_id = get_or_insert_id(
                        ...
                        "airports",
                        "airport_name",
                        layover_info["Layover Airport"],
                        "airport_id",
                        {"city": layover_info["Layover City"]},
                    )
                    insert_layover(
                        cursor,
                        flight_id,
                        layover_airport_id,
                        layover_info["Layover Duration"],
                    )
                    connection.commit()
...
def userControl(dict):
    """
    Automates interaction with the Google Flights Explore page based on user parameters.

    Parameters:
        origin (str): Origin airport or city.
        destination (str): Destination airport or city.
        round_trip (bool): True for round trip, False for one-way.
        num_adults (int): Number of adults.
        num_children (int): Number of children.
        num_infants_in_seat (int): Number of infants in seats.
        num_infants_on_lap (int): Number of infants on laps.
        seating_class (str): Seating class (Economy, Prem Econ, Business, First Class).
        departure_date (str): Departure date in MM/DD/YYYY format (for specific dates).
        return_date (str): Return date in MM/DD/YYYY format (for specific dates).
        flexible_date (bool): True for flexible dates, False for specific dates.
        flex_month (str): Month for flexible dates (e.g., "January").
        flex_duration (str): Duration for flexible dates (e.g., "Weekend", "1 Week", "2 Weeks").
    """
    # Parse Dictionary
    origin = dict["departure"]
    destination = dict["arrival"]
    departure_date = dict["depart_date"]
    round_trip = dict["roundtrip"]
    seating_class = dict["seat_class"]
    ...
    # Set the seating class
    accessSeatingClass(wait, driver, seating_class)

    # Log the interaction
    createLog(f"User: Interaction for {origin} to {destination} on {departure_date}")

    # Set the number of passengers
    # accessNumOfPassengers(
    #    wait, driver, num_adults, num_children, num_infants_in_seat, num_infants_on_lap
    # )

    # Set the dates (flexible or specific)
    # if flexible_date:
    #    accessFlexibleDates(wait, driver, flex_month, flex_duration)
    # else:
    #    accessSpecificDates(wait, driver, departure_date, return_date)

    # Access origin and destination
    accessOriginDestination(wait, driver, origin, destination)
...
def driver():
    ...
    date1, date2, flexible_date = (
        getUserDate()
    )  # date1 = user_month, date2 = user_length, flexible_date = True| or | # date1 = departure_date, date2 = arrival_date # flexible_date = False
    if flexible_date:
    ...
    dest = "Denver"
    num_adults, num_children, num_infants_in_seat, num_infants_on_lap = (
        2,
        1,
        0,
        0,
    ...
    accessNumOfPassengers(
        wait, driver, num_adults, num_children, num_infants_in_seat, num_infants_on_lap
    )  # Access the number of passengers for user
    ...
    data = [
        ...
        "1 stop flight",
        "with Frontier",
        "Leaves Baltimore/Washington International Thurgood Marshall Airport at 6:44 PM on Sunday, January 5",
        "arrives at Denver International Airport at 7:04 AM on Monday, January 6",
        "Total duration 14 hr 20 min",
        "Layover (1 of 1)",
        "is a 8 hr 22 min",
        "layover at Hartsfield-Jackson Atlanta International Airport in Atlanta",
        "0 carry-on bags",
        "0 checked bags",
    ]
